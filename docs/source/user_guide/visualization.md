...
`cvzx.visualize_base_gates.visualize(diagram, title="", config=None) -> matplotlib.figure.Figure`
...
Pass a `cvzx.visualize_base_gates.VisualizerConfig` to `visualize()` to control node size,
...
```python
from cvzx.visualize_base_gates import VisualizerConfig, visualize
...
For repeated use, `cvzx.visualize_base_gates.DiagramVisualizer(config)` holds a config and
exposes the same `.visualize(diagram, title)` method `visualize()` delegates to.

## Before/after comparisons

`cvzx.visualize_base_gates.visualize_before_after(diagram_before, diagram_after, test_name,
