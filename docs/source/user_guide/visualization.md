...
`cvzx.visualization.core.visualize(diagram, title="", config=None) -> matplotlib.figure.Figure`
...
Pass a `cvzx.visualization.core.VisualizerConfig` to `visualize()` to control node size,
...
```python
from cvzx.visualization.core import VisualizerConfig, visualize
...
For repeated use, `cvzx.visualization.core.DiagramVisualizer(config)` holds a config and
exposes the same `.visualize(diagram, title)` method `visualize()` delegates to.

## Before/after comparisons

`cvzx.visualization.debug.visualize_before_after(diagram_before, diagram_after, test_name,
