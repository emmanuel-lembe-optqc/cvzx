...
through the same per-leaf translators `cvzx.diagram_to_circuit` already
...
2. Every `input_states` node starts a fresh mode (a monotonically
   increasing integer, exactly as `cvzx.diagram_to_circuit`'s own
...
from cvzx.base_gates import Diagram, VoidDiagram
from cvzx.completion import complete_boundaries
from cvzx.config import Backend
from cvzx.diagram_to_circuit import MeasurementOps, _apply_1mode_leaf, _apply_2mode_leaf, _open_mode_state
...
def extract_dependency_dag(
    ...
    *,
    ...
    `"cvzx-direct"` in `cvzx.lowering`'s backend registry) that discovers
