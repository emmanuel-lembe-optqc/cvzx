...
class TestGraphConversion:
    ...
    def _assert_diagram_equality(self, original: Diagram, reconstructed: Diagram):
        ...
        if isinstance(original, CompositionDiagram):
            ...
            for i, (orig_sub, recon_sub) in enumerate(zip(original.diagrams, reconstructed.diagrams)):
    ...
    def _assert_roundtrip(self, diagram: Diagram):
        """Test roundtrip conversion: diagram → graph → diagram."""
        G = to_graph(diagram)
        reconstructed = to_diagram(G)
    ...
    def test_proper_qspider(self):
        ...
        assert attrs["diagram"] is self.q1
    ...
    def test_proper_fourier(self):
        ...
        G = to_graph(self.fourier)

        assert G.number_of_nodes() == 1
        attrs = G.nodes[self.fourier.id]
    ...
    def test_proper_swap(self):
        ...
        G = to_graph(self.swap)

        assert G.number_of_nodes() == 1
        attrs = G.nodes[self.swap.id]
    ...
    def test_proper_multi_arity_spider(self):
        ...
        G = to_graph(self.q_1x2)

        assert G.number_of_nodes() == 1
        attrs = G.nodes[self.q_1x2.id]
        assert attrs["n_inputs"] == 1
        assert attrs["n_outputs"] == 2
    ...
    def test_composition_two_proper_diagrams(self):
        ...
        assert G.has_node(self.q1.id)
        assert G.has_node(self.q2.id)
        assert G.has_node(comp.id)

        # Check edges: q1 → q2
        assert G.number_of_edges() == 1
        edge_data = G.get_edge_data(self.q1.id, self.q2.id)
        ...
        assert G.nodes[self.q1.id]["container_id"] == comp.id
        assert G.nodes[self.q2.id]["container_id"] == comp.id
    ...
    def test_composition_three_proper_diagrams(self):
        ...
        assert G.number_of_nodes() == 4  # 3 proper + 1 container
        ...
        # q2 → q3
        edge_data = G.get_edge_data(self.q2.id, self.q3.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [0]
    ...
    def test_composition_proper_with_connectivity(self):
        ...
        edge_data = G.get_edge_data(self.q_1x2.id, self.q_2x1.id)
    ...
    def test_composition_mixed_proper_types(self):
        ...
        assert G.number_of_edges() == 2
        assert G.has_edge(self.q1.id, self.fourier.id)
        assert G.has_edge(self.fourier.id, self.q2.id)
    ...
    def test_tensor_proper_diagrams(self):
        ...
        assert G.has_node(self.q1.id)
        assert G.has_node(self.q2.id)
        assert G.has_node(self.q3.id)
        assert G.has_node(tensor.id)

        # No edges between tensor components
        assert G.number_of_edges() == 0

        # Check tensor container attributes
        attrs = G.nodes[tensor.id]
        assert attrs["kind"] == "container"
    ...
    def test_composition_of_tensors1(self):
        ...
        assert G.number_of_edges() == 2
        edge_data = G.get_edge_data(self.q1.id, self.q3.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [0]
        edge_data = G.get_edge_data(self.q2.id, self.q4.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [0]
    ...
    def test_composition_of_tensors2(self):
        ...
        tensor_attrs = G.nodes[tensor_node]
        ...
        for f_node in [self.fourier.id, self.fourier.id]:
            assert G.nodes[f_node]["container_id"] == tensor_node

        # Swap node should be inside the composition
        assert G.nodes[self.swap.id]["container_id"] == comp_node

        # Find the Fourier nodes in the graph
        fourier_nodes = [
            n for n, attrs in G.nodes(data=True) if "Fourier" in attrs.get("type") and attrs.get("kind") == "proper"
        ]
        ...
        edge_data = G.get_edge_data(self.swap.id, fourier_nodes[0])
        ...
        edge_data = G.get_edge_data(self.swap.id, fourier_nodes[1])
        assert edge_data is not None
        assert "source_ports" in edge_data
        assert "target_ports" in edge_data
        assert edge_data["source_ports"] == [0]
    ...
    def test_tensor_of_compositions(self):
        ...
        assert G.number_of_edges() == 2  # q1→q2 and q3→q4
    ...
    def test_contracted_proper_diagrams(self):
        ...
        assert G.has_edge(self.q_1x2.id, self.p_2x1.id)
        edge_data = G.get_edge_data(self.q_1x2.id, self.p_2x1.id)
        ...
        assert G.has_edge(self.p_2x1.id, self.q_1x2.id)
        edge_data = G.get_edge_data(self.p_2x1.id, self.q_1x2.id)
    ...
    def test_composition_of_contracted_diagrams(self):
        ...
        assert G.has_edge(self.q2.id, self.q3.id)

        # Check hierarchy
        comp_attrs = G.nodes[comp.id]
        assert comp_attrs["sub_diagram_ids"] == [contracted1.id, contracted2.id]

        contracted1_attrs = G.nodes[contracted1.id]
        ...
        contracted2_attrs = G.nodes[contracted2.id]
    ...
    def test_tensor_of_contracted_diagrams(self):
        ...
        assert G.number_of_edges() == 2  # internal edges only
    ...
    def test_complex_nested_structure1(self):
        ...
        contracted_attrs = G.nodes[contracted.id]
        assert contracted_attrs["container_id"] == tensor.id

        # Edges: q1→q2 (inside comp)
        assert G.has_edge(self.q1.id, self.q2.id)
        edge_data = G.get_edge_data(self.q1.id, self.q2.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [0]

        # Internal edges inside contracted
        assert G.has_edge(self.q3.id, self.q4.id)
        edge_data = G.get_edge_data(self.q3.id, self.q4.id)
        assert edge_data["edge_type"] == "contracted_internal"
        assert edge_data["internal"] is True
        assert edge_data["connection_type"] == "I1_I2"
    ...
    def test_complex_nested_structure2(self):
        ...
        outer_tensor_attrs = G.nodes[root]
        ...
        # Check composition
        connectivity = get_connectivity(G, comp_node)
        assert connectivity is not None
        assert connectivity == large_comp_conn.connectivity
        assert len(G.nodes[comp_node]["sub_diagram_ids"]) == 8

        # Find tensor containers
        tensor_nodes = [n for n, attrs in G.nodes(data=True) if attrs.get("container_type") == "tensor" and n != root]
        assert len(tensor_nodes) == 4

        # Check large_tensor (first sub-diagram of composition)
        large_tensor_node = None
        for n in tensor_nodes:
            if G.nodes[n].get("sub_diagram_ids") == [fourier.id, fourier_squared.id, fourier_inv.id]:
                large_tensor_node = n
                break
        assert large_tensor_node is not None
        assert G.nodes[large_tensor_node]["container_id"] == comp_node

        # Check swap_tensor (second and third sub-diagrams of composition)
        swap_tensor_nodes = [n for n in tensor_nodes if len(G.nodes[n].get("sub_diagram_ids")) == 2]
        assert len(swap_tensor_nodes) == 3
        for n in swap_tensor_nodes:
            assert G.nodes[n]["container_id"] == comp_node
        ...
        assert G.nodes[contracted_node]["container_id"] == comp_node
        assert G.nodes[contracted_node]["first_id"] == q_spider_5x5.id
        assert G.nodes[contracted_node]["second_id"] == p_spider_4x4.id

        # Check contracted connections
        contracted_connections = get_contracted_connections(G, contracted_node)
        ...
        contracted_attrs = G.nodes[contracted_node]
        ...
        assert G.has_edge(q_spider2.id, swap_node1.id)
        edge_data = G.get_edge_data(q_spider2.id, swap_node1.id)
        assert edge_data["source_ports"] == [0, 2]
        assert edge_data["target_ports"] == [1, 0]

        assert G.has_edge(q_spider2.id, fourier2.id)
        edge_data = G.get_edge_data(q_spider2.id, fourier2.id)
        assert edge_data["source_ports"] == [1]
        assert edge_data["target_ports"] == [0]

        # Check Connexion tensor 1 -> large_tensor_conn
        assert G.has_edge(swap_node1.id, fourier.id)
        edge_data = G.get_edge_data(swap_node1.id, fourier.id)
        assert edge_data["source_ports"] == [1]
        assert edge_data["target_ports"] == [0]

        assert G.has_edge(swap_node1.id, fourier_inv.id)
        edge_data = G.get_edge_data(swap_node1.id, fourier_inv.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [0]

        assert G.has_edge(fourier2.id, fourier_squared.id)
        edge_data = G.get_edge_data(fourier2.id, fourier_squared.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [0]

        # Check Connexion large_tensor_conn -> tensor2
        assert G.has_edge(fourier_squared.id, swap_node2.id)
        edge_data = G.get_edge_data(fourier_squared.id, swap_node2.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [0]

        assert G.has_edge(fourier_inv.id, fourier_squared2.id)
        edge_data = G.get_edge_data(fourier_squared.id, swap_node2.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [0]

        assert G.has_edge(fourier.id, swap_node2.id)
        edge_data = G.get_edge_data(fourier.id, swap_node2.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [1]

        # Check Connection tensor2 → tensor3
        assert G.has_edge(swap_node2.id, swap_node3.id)
        edge_data = G.get_edge_data(swap_node2.id, swap_node3.id)
        assert edge_data["source_ports"] == [0, 1]
        assert edge_data["target_ports"] == [0, 1]

        assert G.has_edge(fourier_squared2.id, fourier_inv2.id)
        edge_data = G.get_edge_data(fourier_squared2.id, fourier_inv2.id)
        assert edge_data["source_ports"] == [0]
        assert edge_data["target_ports"] == [0]
    ...
    def test_root_node_identification(self):
        ...
        comp = CompositionDiagram([self.q1, self.q2])
        G = to_graph(comp)
        assert get_root_node(G) == comp.id

        # Root is tensor
        tensor = TensorDiagram([self.q1, self.q2])
        G = to_graph(tensor)
        assert get_root_node(G) == tensor.id

        # Root is contracted
        contracted = ContractedDiagram(self.q1, self.q2, [0], [0], [], [])
        G = to_graph(contracted)
        assert get_root_node(G) == contracted.id
    ...
    def test_container_id_tracking(self):
        ...
        assert get_immediate_container(G, self.swap.id) == comp.id
        ...
        assert get_immediate_container(G, self.q2.id) == tensor_id
        assert get_immediate_container(G, self.q3.id) == tensor_id

        # Tensor is in comp
        assert get_immediate_container(G, tensor_id) == comp.id

        # Comp has no container
        assert get_immediate_container(G, comp.id) is None
    ...
    def test_get_nodes_by_container(self):
        ...
        nodes_in_tensor = get_nodes_by_container(G, tensor_id)
        ...
        nodes_in_comp = get_nodes_by_container(G, comp.id)
        print(nodes_in_comp)
    ...
    def test_get_connectivity(self):
        ...
        retrieved_conn = get_connectivity(G, comp.id)
        assert retrieved_conn[0] == conn

        # Non-composition container returns None
        tensor = TensorDiagram([self.q1, self.q2])
        G = to_graph(tensor)
        assert get_connectivity(G, tensor.id) is None
