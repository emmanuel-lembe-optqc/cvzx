"""Unit tests for `PassthroughRule` using the graph formalism.
...
from cvzx.backends.nx.rules import PassthroughRule, apply_rule_to_diagram
...
class TestPassthroughRuleWiring(unittest.TestCase):
    ...
    def test_case_a_swap_routes_real_value_to_successor_b(self):
        ...
        swap_id = next(n for n, d in graph.nodes(data=True) if d.get("type") == "Swap")
        survivor_id = next(
            n for n, d in graph.nodes(data=True) if d.get("type") == "QSpider" and d.get("kind") == "proper"
        )
        fourier_inv_id = next(n for n, d in graph.nodes(data=True) if d.get("type") == "FourierInv")

        assert graph[survivor_id][swap_id]["source_ports"] == [0]
        assert graph[survivor_id][swap_id]["target_ports"] == [0]
        assert graph[swap_id][fourier_inv_id]["source_ports"] == [0]
        assert graph.nodes[swap_id]["void_input_port"] == 0
    ...
    def test_case_b_swap_routes_real_value_to_survivor(self):
        ...
        swap_id = next(n for n, d in graph.nodes(data=True) if d.get("type") == "Swap")
        survivor_id = next(
            n for n, d in graph.nodes(data=True) if d.get("type") == "PSpider" and d.get("kind") == "proper"
        )
        fourier_id = next(n for n, d in graph.nodes(data=True) if d.get("type") == "Fourier")

        assert graph[fourier_id][swap_id]["target_ports"] == [0]
        assert graph[swap_id][survivor_id]["source_ports"] == [1]
        assert graph.nodes[swap_id]["void_input_port"] == 1
