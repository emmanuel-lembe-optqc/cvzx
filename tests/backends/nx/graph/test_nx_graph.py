...
class TestCVZXGraphParameterConsistency:
    ...
    def test_symbol_conflict_violation(self):
        ...
        graph.add_node(100, kind="proper", num_inputs=1, num_outputs=0)
        graph.add_node(200, kind="proper", num_inputs=1, num_outputs=0)
        cvzx_graph = CVZXGraph(graph)
...
class TestToGraphDoesNotMutateInput:
    ...
    of `cvzx.backends.nx.graph` -- it walks the `Diagram` tree directly via
    ...
    def _snapshot(self, diagram: Diagram) -> str:
        """A recursive, `cvzx.backends.nx.graph`-independent snapshot of every object.
