"""Unit tests for the identity rewrite rule using the graph formalism.

These tests verify the IdentityRule implementation on graphs, including matching,
...
class TestIdentityRule(unittest.TestCase):
    """Test suite for IdentityRule (graph-based)."""
    ...
    def test_apply_single_wrong_match(self):
        ...
        comp = CompositionDiagram([self.id_q, self.fourier])
        graph = to_graph(comp)
        self.rule.apply_single(
            graph, {"node_id": self.fourier.id, "container_id": graph.graph.nodes[self.fourier.id]["container_id"]}
        )
    ...
    def test_apply_single_identity_in_tensor_does_nothing(self):
        ...
        graph = to_graph(tensor)
        self.rule.apply_single(
            graph, {"node_id": self.id_q.id, "container_id": graph.graph.nodes[self.id_q.id]["container_id"]}
        )
    ...
    def test_apply_rule_tensor3(self):
        ...
        # comp3 -> [non_id_q_4x4, non_id_q_4x4, TensorDiagram([beam_splitter1, fourier_inv, ph_rot])]
...
if __name__ == "__main__":
    ...
    comp_deep = CompositionDiagram([
        non_id_q_3x3,
        TensorDiagram([beam_splitter1, CompositionDiagram([id_q, fourier, id_p])]),
        TensorDiagram([
            CompositionDiagram([QSpider(1, 1, zero_phase), sq_gate, PSpider(1, 1, zero_phase)]),
            ph_rot,
            fourier,
        ]),
    ])
    ...
    contracted_after = apply_rule_to_diagram(rule, contracted)
